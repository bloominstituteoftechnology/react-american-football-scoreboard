...
- [Y] Set up the state values for the Lions team score using the state hook
...
- [Y] Set up the state value for the Tigers team score using a second state hook call

### STEP 3 - Render the Scores to the DOM

- [Y] The scores in the JSX are currently hardcoded to 32 points each. Remove the hardcoded values
- [Y] Render the state values from what we just set up in steps 1 and 2
- [Y] Play around with different initial values to test if they are rendering on the DOM correctly

### STEP 4 - Add Click Functionality to Increment the Scores

- [Y] Add the `onClick` handler to each function
- [Y] Determine how much you will need to increment the score for each button
  - [Y] A touchdown is worth 7 points (assume the following extra point is made)
  - [Y] A field goal is worth 3 points
